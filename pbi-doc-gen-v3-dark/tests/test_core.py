...
class TestCIBranding(unittest.TestCase):
    """Tests for CI/Branding and Screenshot models."""

    def test_ci_roundtrip(self):
        from src.models import Project, CIBranding
        p = Project()
        p.ci_branding = CIBranding(
            company_name="Test AG",
            logo_path="/tmp/logo.png",
            primary_color="#112233",
            accent_color="#AABBCC",
            footer_text="Confidential Footer",
            confidentiality_notice="Nur intern",
        )
        d = p.to_dict()
        p2 = Project.from_dict(d)
        self.assertEqual(p2.ci_branding.company_name, "Test AG")
        self.assertEqual(p2.ci_branding.primary_color, "#112233")
        self.assertEqual(p2.ci_branding.confidentiality_notice, "Nur intern")

    def test_screenshot_roundtrip(self):
        from src.models import Project, Screenshot
        p = Project()
        p.screenshots = [Screenshot(filename="test.png", caption="Ein Test", section="overview")]
        d = p.to_dict()
        p2 = Project.from_dict(d)
        self.assertEqual(len(p2.screenshots), 1)
        self.assertEqual(p2.screenshots[0].filename, "test.png")
        self.assertEqual(p2.screenshots[0].section, "overview")

    def test_ci_yaml_persistence(self):
        from src.models import Project, CIBranding
        from src.storage import save_project, load_project
        import tempfile
        from pathlib import Path
        p = Project()
        p.ci_branding.company_name = "YAML Test GmbH"
        p.ci_branding.accent_color = "#FF0000"
        with tempfile.TemporaryDirectory() as td:
            fp = Path(td) / "test.yml"
            save_project(p, fp)
            p2 = load_project(fp)
            self.assertEqual(p2.ci_branding.company_name, "YAML Test GmbH")
            self.assertEqual(p2.ci_branding.accent_color, "#FF0000")
